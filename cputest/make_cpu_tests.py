...
def test_ins_with_modes(ins, modes, val, result=None, p='', ep=None, **kwargs):
    ...
    for mode in modes:
        if mode == A_IMM:
            ...
            do_test('($10,x)', mem=0x7F1212, d=0xffff, x=test_data-0xF, dbr=0x7F)
            if bits == 16:  # test no bank wrapping at destination
                do_test('($10,x)', mem=0x7EFFFF, d=0xff00, x=0x123, memw_0033=0xFFFF, dbr=0x7E, advanced=True)

            # 8 bit index
            do_test('($90,x)', mem=0x7F1212, x=0x1210, d=test_data & 0xFF00, dbr=0x7F, add_flag='X')
...
def write_table():
    add_asm('.segment "RODATA"')
    add_asm('tests_table:')
    for i in range(test_num):
        add_asm(f'.faraddr test{i:04x}\n')
    add_asm('.faraddr success')
...
def handle_cmdline():
    ...
    global test_list_filename
    global test_asm_filename
    global mach
    global test_data
    ...
    while sys.argv[i].startswith("-"):
        arg = sys.argv[i]
        if arg in ('-h', '--help'):
            eprint(f'Usage:  {sys.argv[0]} [--basic|--full] <list filename> <asm filename>')
            eprint()
            eprint('--basic:  Generate basic tests only')
            sys.exit(2)
        elif arg == '--basic':
            basic_tests_only = True
        elif arg == '--full':
            basic_tests_only = False
        elif arg.startswith('--mach='):
            mach = arg[7:]
        elif arg.startswith('--test_data='):
            test_data = int(arg[12:], 16)
        else:
            eprint(f"Unknown command line argument : {arg}")
            sys.exit(2)
        i=i+1
    ...
    test_list_filename = sys.argv[i]
    test_asm_filename = sys.argv[i+1]


def main():
    global TESTS_IN_BANK0
    global TESTS_PER_BANK

    handle_cmdline()

    # Limit tests per bank to not exceed 32KB. Bank 0 has less room due to other code and data in it.
    if mach == "vhdl816" or mach == "sim816":
        TESTS_IN_BANK0 = 220
        TESTS_PER_BANK = 720
    else:
        TESTS_IN_BANK0 = 230
    ...
    add_asm('.export tests_table')
    add_asm('.export start_tests')

    add_asm('.a16')
    add_asm('.i16')
    add_asm('.import success:far')
    add_asm('.import fail:far')
    add_asm('.import save_results:far')
    add_asm('.import init_test:far')

    add_asm('.importzp test_num')
    add_asm('.importzp result_a')
    add_asm('.importzp result_x')
    add_asm('.importzp result_y')
    add_asm('.importzp result_p')
    add_asm('.importzp result_s')
    add_asm('.importzp result_d')
    add_asm('.importzp result_dbr')
    add_asm('.importzp retaddr')
    ...
    with open(test_asm_filename, 'w') as f:
        f.write('\n'.join(asm_code) + '\n')
